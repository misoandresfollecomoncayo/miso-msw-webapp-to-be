...
$service->setCallback(function() use ($service) {
    ...
    $res = MswApiClient::request("PATCH", "/api/customers/" . rawurlencode($id) . "/active", array("active" => true));
    if (MswApiClient::isOk($res)) {
        $service->setResponse("Cliente activado correctamente.");
    } else if ($res["status"] == 404) {
        $service->setException("Cliente no existe.");
    } else {
        $service->setException(MswApiClient::errorMessage($res, "No se pudo activar el cliente."));
    }
});
$service->publish();
