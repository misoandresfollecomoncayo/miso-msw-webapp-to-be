...
$service->setCallback(function() use ($service) {
    $UUID = EcuadorDAO::edit($service->getParameter("Id")->getValue(), $service->getParameter("Bill")->getValue(), $service->getParameter("Customer")->getValue());
    
    EcuadorDAO::deleteItemsPending($service->getParameter("Id")->getValue());
    
    $items = json_decode($service->getParameter("Items")->getValue());
    
    foreach ($items as $i) {
        EcuadorDAO::insertItem($i->quantity, $i->sequence, $i->description, $service->getParameter("Id")->getValue());
    }
    
    $service->setResponse("Registros actualizados correctamente.");
});
$service->publish();