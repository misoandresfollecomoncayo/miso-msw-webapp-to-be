...
$service->addParameterObj(new CloudEngineWebServiceParameterText("IdShipmentCompany", 11, false));
...
$service->setCallback(function() use ($service) {
    ...
    if (count($purchases) > 0) {
        if (is_numeric($TRM) && is_numeric($poundValue) && is_numeric($volumetricPoundValue) && is_numeric($declaredValue) && is_numeric($secure) && is_numeric($additionalValue)) {
            ...
            $service->setResponse("Envío actualizado correctamente.");
        } else {
            $service->setException("Valor libra, valor libra volumétrica, valor declarado, seguro y valor adicional, deben ser números.");
        }
    } else {
