<?php
    ...
    foreach ($ids as $id) {
        array_push($shipments, ShippingDAO::getShippingById($id));
    }

    foreach ($shipments as $s) {
        if ($s->getCurrency() == "USD") {
            $totalUSD += $s->getPendingPayment();
        } else {
            $totalCOP += $s->getPendingPayment();
        }
    }
...
?>
    <body>
        ...
        <div class="float-left canvas-width height-100 background-color-light-gray mobile-width-100">
            <?php $layout->printSessionBar("Pago masivo", PUBLIC_PATH_PLATFORM . "Shipments/index.php") ; ?>
                    ...
                    <table id="tblItems" class="table">
                        ...
                        <tbody>
                            <?php
                                ...
                                foreach ($shipments as $s) {
                                    $code = "<tr data-id='" . $s->getIdShipping() . "'>";
                                    $code .= "<td class='text-align-center'>" . $s->getShippingNumber() . "</td>";
                                    $code .= "<td class='text-align-center'>" . $s->getPurchases()[0]->getCustomer()->getNames() . "</td>";
                                    $code .= "<td class='text-align-center'>" . $s->getPurchases()[0]->getCustomer()->getLockerNumber() . "</td>";
                                    $code .= "<td class='text-align-center'>$ " . number_format($s->getPendingPayment(), 2) . " " . $s->getCurrency(); "</td>";
                                    $code .= "<td><input id='" . rand(0, 999) . "' data-required='true' data-name='Pagar' name='amount' class='input-text-underline' type='number' value='" . $s->getPendingPayment() . "' /></td>";
        ...
        <script type="text/javascript">
            $("#btnElectronicPayment").on("click", function(e) {
                ...
                for (var i=0; i<rows.length; i++) {
                    var r = $(rows[i]);
                    entities.push(new Entity(r.data("id"), "SHIPMENT"));
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (index < items.length-1) {
                            ...
                            document.location.href = "/Platform/Shipments/index.php";
                        }
                    }
                });
            }
