<?php
    ...
    $items = $product->getTracking();
?>
<div id='inventoryAddTrackingPopup' class="popup" style="display: flex; flex-direction: column; gap: 20px">
    <div style="font-size: 20px; font-weight: bold">Trazabilidad: <?php echo $product->fullInvoiceCode; ?></div>
        ...
        methods: {
            save() {
                if (confirm("¿Confirma registrar el movimiento?")) {
                    showPreload();
                    var fd = new FormData();
                    fd.append("Id", "<?php echo $product->id; ?>");
                    fd.append("Date", this.date);
                    fd.append("Detail", this.detail);
                    this.$http.post(URL_API + "Inventory/Tracking.php", fd).then(function(response){
