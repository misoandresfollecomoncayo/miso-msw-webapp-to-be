<?php
    ...
    $product = InventoryDAO::getById($_REQUEST["Id"]);
    
    $layout = new Layout();
    $layout->setTitle("Editar producto");
    $layout->printHead();
?>
    <body>
        <?php $layout->printMainBar(); ?>
        <div id="root" class="float-left canvas-width height-100 background-color-light-gray mobile-width-100">
            <?php $layout->printSessionBar("Editar producto", PUBLIC_PATH_PLATFORM . "Inventory"); ?>
        ...
        <script type="text/javascript">
            let app = new Vue({
                el: "#root",
                data: {
                    model: <?php echo json_encode($product); ?>,
                    id: "",
                    product: "",
                    trm: 0,
                    usdPrice: 0,
                ...
                mounted() {
                    ...
                    this.trm = this.model.trm;
                    this.usdPrice = this.model.usdPrice;
                    this.internationalShippingPrice = this.model.internationalShippingPrice;
                    this.nationalShippingPrice = this.model.nationalShippingPrice;
                    this.salePrice = this.model.salePrice;
                },
                computed: {
                    computedCOPPrice() {
                        return parseFloat(this.trm * this.usdPrice).toFixed(0);
                    },
                    computedTotalCost() {
                        return parseFloat(parseFloat(this.computedCOPPrice) + parseFloat(this.internationalShippingPrice) + parseFloat(this.nationalShippingPrice)).toFixed(0);
                    },
                    computedUtility() {
                        return parseFloat(this.salePrice - this.computedTotalCost).toFixed(0);
                    }
                },
                methods: {
                    ...
                    async save() {
                        if (confirm("¿Confirma guardar el producto?")) {
                            var fd = new FormData();
                            ...
                            this.$http.post(URL_API + "Inventory/Update.php", fd).then(function(response) {
